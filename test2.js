...
function renderKategoriPage(container) {
    ...
    kategoriData.forEach(function(kat, idx) {
        rows += '<tr><td>' + (idx+1) + '</td><td><i class="' + (kat.icon||'fa-solid fa-folder') + '" style="margin-right:8px; color:var(--primary-red);"></i>' + kat.nama + '</td>' +
            ...
            '<td><div class="action-btns"><button class="btn-icon btn-edit" onclick="editKategori(\'' + kat.id + '\', \'' + kat.nama.replace(/'/g, "\\'") + '\', \'' + (kat.icon||'') + '\', \'' + (kat.kode_arsip||'') + '\')"><i class="fa-solid fa-pen"></i></button><button class="btn-icon btn-delete" onclick="confirmHapusKategori(\'' + kat.id + '\', \'' + kat.nama.replace(/'/g, "\\'") + '\')" ><i class="fa-solid fa-trash-can"></i></button></div></td></tr>';
    });
...
function renderSubKategoriPage(container) {
    ...
    if (allSubs.length === 0) rows = '<tr><td colspan="5" style="text-align:center; padding:30px; color:var(--text-muted);">Belum ada sub-kategori.</td></tr>';
        ...
        rows += '<tr><td>' + (i+1) + '</td><td>' + s.kategoriNama + '</td><td>' + s.nama + '</td>' +
            ...
            '<td><button class="btn-icon btn-delete" onclick="confirmHapusSubKat(\'' + s.id + '\', \'' + s.nama.replace(/'/g, "\\'") + '\')"><i class="fa-solid fa-trash-can"></i></button></td></tr>';
    });
